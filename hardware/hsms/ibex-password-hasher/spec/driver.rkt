...
(define (wait-until-start-bit)
  ...
  (hint maybe-merge-after-read)
  (hint maybe-merge-after-write)
  (if (output-tx (in))
      (begin
        (tick)
        (hint maybe-concretize-cpu)
        (hint maybe-split-read)
        (hint maybe-split-write)
        (hint debug)
        (hint sync)
...
(define (repeat0 n)
  (if (<= n 0)
      (list)
      (cons (bv 0 8) (repeat0 (sub1 n)))))

(define (handle cmd)
  (reset)
  (send-bytes cmd)
  (wait-and-return (recv-bytes 33)))
