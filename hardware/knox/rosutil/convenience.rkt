...
(define-syntax (define-singleton stx)
  (syntax-parse stx
    [(_ name:id)
     ...
     #'(@begin
        (@define name (@let () (@struct name () #:transparent) (name)))
        (@define (name? x) (@equal? x name)))]))

(define (choose . xs)
  (@cond
    [(@empty? xs) (error 'choose "no choices")]
    [(@empty? (@rest xs)) (@first xs)]
    [@else (@if (fresh-symbolic "choose" @boolean?) (@first xs) (@apply choose (@rest xs)))]))
