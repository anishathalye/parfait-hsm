...
(define STATE-LENGTH 14) ; 32-bit words
(define COMMAND-LENGTH 49) ; bytes
(define RESPONSE-LENGTH 65) ; bytes
...
(define (do-read-state!)
  ...
  (step-until! (pc-is (bv #x258 32))) ; li a2, 56
  ...
  (define (align-case!)
    ...
    (step-until! (pc-is (bv #x27c 32))) ; ret
    ...
    (overapproximate! (lens 'circuit '|wrapper.soc.cpu.u_ibex_core.id_stage_i.imd_val_q [67:34]|))
    (overapproximate-predicate! #t))
...
(define (do-handle!)
  (step-until! (pc-is (bv #xbc8 32))) ;; start of impl_main
  ...
  (define (ready? t)
    ...
    (and (not ((mapping-symbolic-branch? ibex-mapping) emu-ckt))
         (bvuge (get-field (pairing-circuit (set-term t)) 'wrapper.soc.cpu.u_ibex_core.if_stage_i.pc_id_o) (bv #xbec 32))))
  (define (all-ready?)
...
(define (do-write-state!)
  (step-until! (pc-is (bv #x2d8 32))) ; li a1, 1
  (define state (lens-view (lens 'emulator 'oracle) (set-term (current))))
  ...
  (define (align-case!)
    ...
    (step-until! (pc-is (bv #x318 32))) ; ret
    ;; do alignment
    (for ([i (range STATE-LENGTH)])
