...
(define (step)
  ...
  (let ([c (get)])
    (if (and
         ...
         (equal? (get-field c 'wrapper.soc.cpu.u_ibex_core.if_stage_i.pc_id_o) (bv #xbc8 32)))
  ...
  (let ([c (get)])
    (if (and
         (equal? (get-field c 'wrapper.pwrmgr_state) (bv #b10 2))
         (equal? (get-field c 'wrapper.soc.cpu.u_ibex_core.if_stage_i.pc_id_o) (bv #xbe0 32))) ; beq ...
        (let ([ram (get-field c 'wrapper.soc.ram.ram)])
  ...
  (let ([c (get)])
    (if (and
         (equal? (get-field c 'wrapper.pwrmgr_state) (bv #b10 2))
         (equal? (get-field c 'wrapper.soc.cpu.u_ibex_core.if_stage_i.pc_id_o) (bv #xb80 32)))
        (let ()
