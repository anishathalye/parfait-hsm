...
(define (step)
  ...
  (let ([c (get)])
    (if (and
         ...
         (equal? (get-field c 'wrapper.soc.cpu.u_ibex_core.if_stage_i.pc_id_o) (bv #x618 32)))
  ...
  (let ([c (get)])
    (if (and
         (equal? (get-field c 'wrapper.pwrmgr_state) (bv #b10 2))
         (equal? (get-field c 'wrapper.soc.cpu.u_ibex_core.if_stage_i.pc_id_o) (bv #x630 32))) ; beq ...
        (let ([ram (get-field c 'wrapper.soc.ram.ram)])
