...
(require
 ...
 (prefix-in crv32: (only-in crv32 run))
...
(addressable-struct executing (persistent machine)) ;; when oracle is in the progress of being executed
...
;; oracle api is a parfait:spec
;;
;; defines two functions, call() and return(), for staged execution
(define (oracle->environment oracle-api)
  (list
   (cons 'call (raw (lambda (args st globals)
                      (when (executing? (state-oracle st))
                        (error 'call "oracle already executing"))
                      (result (void)
                              (state (state-auxiliary st)
                                     (executing (state-oracle st)
                                                ((parfait:spec-handle oracle-api)
                                                 (state-oracle st)
                                                 (first args))))))))
   (cons 'return (raw (lambda (args st globals)
                        (when (not (executing? (state-oracle st)))
                          (error 'return "oracle not executing"))
                        ;; run to completion in case proof hasn't done so already
                        (define m (crv32:run
                                   (executing-machine (state-oracle st))
                                   (parfait:spec-environment oracle-api)))
                        (define v ((parfait:spec-get-response oracle-api) m))
                        (define s ((parfait:spec-get-state oracle-api) m))
                        (result v (state (state-auxiliary st) s)))))))
