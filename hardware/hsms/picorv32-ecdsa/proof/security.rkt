...
(define (do-read-state!)
  ...
  (define (align-case!)
    ...
    (overapproximate! (lens 'circuit (list 'wrapper.soc.cpu.mem_wdata
                                           'wrapper.soc.cpu.genblk1.pcpi_mul.rd
                                           'wrapper.soc.cpu.genblk1.pcpi_mul.rs1)))
...
(define (do-handle!)
  ...
  (define (ready? t)
    ...
    (and (not ((mapping-symbolic-branch? picorv32-mapping) emu-ckt))
         (bvuge (get-field (pairing-circuit (set-term t)) 'wrapper.soc.cpu.reg_pc) (bv #xbec 32))))
  (define (all-ready?)
