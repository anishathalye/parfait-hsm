...
(provide
 ...
 (except-out (struct-out spec-finish!) spec-finish!)
 (except-out (struct-out begin-sync!) begin-sync!)
 (except-out (struct-out auto-sync!) auto-sync!)
 (rename-out
  [tactic# tactic]
  [get-state# get-state]
  [merge!# merge!]
...
(struct spec-finish! hint (k))

(define (spec-finish!-hint)
  (wrap (spec-finish!)))

(define-match-expander spec-finish!#
  (syntax-parser [(_ args ...) #'(spec-finish! args ...)])
  (syntax-parser
    [(_ args ...) #'(spec-finish!-hint args ...)]
    [_ #'spec-finish!-hint]))

(struct begin-sync! hint (mapping k))

(define (begin-sync!-hint mapping)
  (wrap (begin-sync! mapping)))

(define-match-expander begin-sync!#
  (syntax-parser [(_ args ...) #'(begin-sync! args ...)])
  (syntax-parser
    [(_ args ...) #'(begin-sync!-hint args ...)]
    [_ #'begin-sync!-hint]))

(struct auto-sync! hint (k))

(define (auto-sync!-hint)
  (wrap (auto-sync!)))

(define-match-expander auto-sync!#
  (syntax-parser [(_ args ...) #'(auto-sync! args ...)])
  (syntax-parser
    [(_ args ...) #'(auto-sync!-hint args ...)]
    [_ #'auto-sync!-hint]))
